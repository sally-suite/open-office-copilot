                        ...
                        />
                        {
                            (agentTools && agentTools.length > 0) && (
                                <div className='flex flex-row items-center text-gray-500 text-sm  whitespace-nowrap  px-1 cursor-pointer'>
                                    {t('common.tools')}:{" "}
                                    {
                                        agentTools.length <= 3 && agentTools?.map((item) => {
                                            return t(`tool:${item.id}`);
                                        }).join(' , ')
                                    }
                                    {
                                        agentTools.length > 3 && `${agentTools.length}`
                                    }
                        ...
                        canUseTool && plugin.tools && plugin.tools.length > 0 && (
                            <div>
                                ...
                                <DropdownMenuGroup className={cn(
                                    plugin.tools?.length >= 4 ? 'w-[735px] grid grid-cols-4 gap-1 text-sm' : 'flex flex-row space-x-1 text-sm',
                                )}>
                                    {
                                        plugin.tools?.map((id) => {
                                            return (
                                                <div key={id}
                                                    className={cn(
                                                        'flex flex-col items-start p-2  min-w-[160px] max-w-[200px] rounded-md cursor-pointer border',
                                                        (agentTools.find(p => p.id == id)) ? 'border border-primary' : ''
                                                    )}
                                                    onClick={onToolItemCheckChange.bind(null, id)}
                                                >
                                                    <div
