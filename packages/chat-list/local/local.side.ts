...
export const getTokenExpireTime = async (): Promise<number> => {
  ...
  return new Promise((resolve) => {
    ...
    if (chrome?.storage?.sync) {
      //@ts-ignore
      chrome.storage.sync.get([USER_TOKEN_EXPIRE], (result) => {
        const time = result[USER_TOKEN_EXPIRE] || 0;
        if (time) {
          return resolve(Number(time));
        }
        resolve(time);
      });
    } else {
...
export const setTokenExpireTime = async (time: number) => {
  ...
  return new Promise((resolve) => {
    ...
    if (chrome?.storage?.sync) {
      //@ts-ignore
      chrome.storage.sync.set({ [USER_TOKEN_EXPIRE]: time }, () => {
...
export const setApiConfig = async (tarModel: string, apiKey: string, baseUrl: string, provider = '') => {
  const apiLocalKey = provider ? `${USER_SET_MODEL_API_KEY}_${tarModel}_${provider}` : `${USER_SET_MODEL_API_KEY}_${tarModel}`;
